        ...
        <div class="col-lg-12" style="margin-top:20px;">
          <div class="table-responsive">
            <table id="tampilDataTables" class="table table-bordered table-hover table-striped tablesorter" cellspacing="0" width="100%">
              ...
              <tbody id="tampilDataDaftarKelasdanWaliKelas">
                <?php
                  $no=1;
                  $tampilDataPegawai = "SELECT * from kelas_transaksi inner join data_pegawai on kelas_transaksi.nip_pegawai_wali_kelas_transaksi = data_pegawai.nip_pegawai order by kd_kelas_daftar_kelas_transaksi asc";
                ...
                <tr>
                  ...
                  <td style="border-radius:0px;margin:0 auto;text-align:center;position:relative;float:center"><?php echo $row["kd_kelas_daftar_kelas_transaksi"] ?></td>
                  <td><?php echo $row["nama_pegawai"] ?></td>
                  <?php
                  $kd_kelas_daftar_kelas_transaksi = $row['kd_kelas_daftar_kelas_transaksi'];
                  $query_hitung_siswa_per_kelas = mysqli_query($connect, "SELECT count(kd_kelas_daftar_siswa) AS jumlah_siswa from data_siswa where kd_kelas_daftar_siswa='$kd_kelas_daftar_kelas_transaksi'");
                  ...
                  <td style="border-radius:0px;margin:0 auto;text-align:center;position:relative;float:center">
                    <a type="button" class="btn btn-success" href="javascript:detailSemuaPengampuMataPelajaran('<?php echo $row['kd_kelas_daftar_kelas_transaksi'] ?>')" ><span class="glyphicon glyphicon-search" aria-hidden="true"></span> Lihat Guru Pengampu</a>
                    <a type="button" class="btn btn-primary" href="javascript:detailSemuaSiswaPerKelas('<?php echo $row['kd_kelas_daftar_kelas_transaksi'] ?>','<?php echo $row['kelompok_kelas_kelas_transaksi'] ?>')" ><span class="glyphicon glyphicon-search" aria-hidden="true"></span> Lihat Siswa</a>
                  </td>
                  <td style="border-radius:0px;margin:0 auto;text-align:center;position:relative;float:center">
                    <a type="button" class="btn btn-primary" href="javascript:editKelas('<?php echo $row['kd_kelas_daftar_kelas_transaksi'] ?>')" ><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                    <a type="button" class="btn btn-danger" href="javascript:deleteKelas('<?php echo $row['kd_kelas_daftar_kelas_transaksi'] ?>','<?php echo $row['kd_kelas_daftar_kelas_transaksi'] ?>')" ><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
                  </td>
                </tr>
                <?php  } ?>
              </tbody>
...
<div id="tambahKelas" class="modal fade">
                ...
                <div class="col-lg-12">
                  <div class="row">
                    <div class="col-lg-12">
                      ...
                      <select class="form-control" name="kd_kelas_daftar_kelas_transaksi" required>
                      ...
                      <div class="col-lg-12" style="margin-top:15px;">
                        ...
                        <select class="form-control" name="nip_pegawai_wali_kelas_transaksi" required>
  ...
  function loadData(){
    $.get('backend/administrator_daftar_kelas_dan_wali_kelas.php',function(data){
      $('#tampilDataDaftarKelasdanWaliKelas').html(data);
    })
  }
  </script> -->
